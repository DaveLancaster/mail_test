...
method url(this: MailTester, domain: string): string {.base.} =
  this.host & "/" & this.version & "/" & domain
method check(this: MailTester, domain: string): Response {.base.} =
  this.client.getContent(this.url(domain)).parseJson().to(Response)
...
proc check*(domain: string): Response =
